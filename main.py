...
class ChatAnalyzer(NcatBotPlugin):
    ...
    def init_config(self):
        """注册配置项"""
        self.register_config("analysis_time", ["22:00"], "分析时间", list)
        self.register_config("analysis_duration", 60 * 24, "分析时长（分钟）", int)
        self.register_config("minimum_message_count", 10, "分析要求的最小消息数", int)
    ...
    async def cmd_analyze(
        self, event: GroupMessageEvent, time: str = "", duration: int = 1440
    ):
        """分析群聊数据并生成图片报告"""
        if not time:
    ...
    async def cmd_test(self, event: GroupMessageEvent):
        ...
        chat_histories = await self._get_chat_history(
            str(event.group_id), "22:00", self.config["analysis_duration"]
        )
        earliest_chat = chat_histories[0]
        if earliest_chat:
            await self.api.post_group_msg(
                event.group_id,
                f"获取到了{len(chat_histories)}条聊天记录喵~最早一条聊天记录是: {earliest_chat.raw_message}，时间是{datetime.fromtimestamp(earliest_chat.time).strftime('%Y-%m-%d %H:%M:%S')}",
            )
    ...
    async def _post_analyze_img(self, group_id: str, time: str, duration: int):
        ...
        # 发送图片
        await self.api.post_group_msg(
            group_id, "大人们，这是你们今天的聊天分析报告，请注意查收喵~"
        )
        await self.api.post_group_msg(group_id, image=f"base64://{img_b64}")

    async def _get_chat_history(
        self, group_id: str, time: str, duration: int, count: int = 101
    ):
        """
        获取指定时间范围内的聊天记录
        ...
        if earliest_timestamp <= start_timestamp:
            ...
            return await self._get_chat_history(
                group_id=group_id, time=time, duration=duration, count=count + 101
            )
    ...
    async def _auto_send_analysis(self, time: str):
        ...
        self.log.info(
            f"触发时间点：{time}，即将向 {'、'.join(subscribed_groups)} 发送聊天分析报告"
        )

        async def task(group_id: str):
            try:
                await self._post_analyze_img(
                    group_id, time, self.config["analysis_duration"]
                )
                ...
                await self.api.post_group_msg(
                    group_id,
                    f"这个时间应该给你们总结近段时间的聊天记录的，结果因为 {e} 没能成功喵……",
                )
    ...
    async def cmd_help(self, event: GroupMessageEvent, command: str = ""):
        ...
        try:
            if not command:
                ...
                if not command_obj:
                    await event.reply(
                        f"未找到指令 {command} 喵，请确认指令名称是否正确喵~"
                    )
                    return
